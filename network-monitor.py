...
def fillUknownMACAddress(uknownHost):
    ...
    t = Timer(timeout, print, [
              '[+] Time\'s up to fill the name about of this addresses'])
...
def scanNetwork(hostList):
    nmp.scan(hosts='192.168.1.0/24',
             arguments='-script smb-os-discovery.nse -T5 192.168.1.0/24')
    i = 0
...
def main():
    ...
    while True:
        ...
        for host in hostList:
            if (host[0] != ""):
                print("		:+: Name -> {} MAC -> {} IPv4 -> {} Vendor -> {} ".format(
